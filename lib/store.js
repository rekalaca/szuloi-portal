...
export const AppStore = {
  ...
  async syncFromCloud() {
    ...
    try {
      ...
      if (!finSnap.empty) {
        ...
        cloudFin.sort((a, b) => this._getItemTimestamp(b) - this._getItemTimestamp(a));
  ...
  getCashFlowY3() {
    ...
    const openingCash = 2600; // 2. tanévről áthozott készpénz záró maradvány
    ...
    finances.forEach((f, idx) => {
      ...
      if (f.paymentMethod === 'cash') {
        if (f.type === 'income') {
          totalIncome += amt;
          let catLabel = 'Osztálypénz';
          if (f.category === 'class_fee' || f.category === 'Osztálypénz' || !f.category) catLabel = 'Osztálypénz';
          else if (f.category === 'trip' || f.category === 'Kirándulás') catLabel = 'Kirándulás';
          else if (f.category === 'arrears' || f.category === 'Elmaradás rendezése') catLabel = 'Elmaradás rendezése';
          else if (f.category === 'other_income' || f.category === 'Egyéb bevétel') catLabel = 'Egyéb bevétel';
          else if (f.category === 'other_expense' || f.category === 'other' || f.category === 'Egyéb') catLabel = 'Egyéb';
          else if (f.category) catLabel = f.category;

          cashIncomes.push({
            id: f.id || `inc-${idx + 1}`,
            date: f.date || '2026/2027. tanév',
            studentName: f.studentName || f.note || 'Tanuló',
            description: f.title || f.note || `${catLabel} készpénzes befizetése`,
            amount: amt,
            formattedAmount: amt.toLocaleString('hu-HU') + ' Ft',
            category: catLabel
          });
        } else if (f.type === 'expense') {
          totalExpenses += amt;
          let catLabel = 'Egyéb';
          if (f.category === 'nameday' || f.category === 'Névnap') catLabel = 'Névnap';
          else if (f.category === 'bank_cost' || f.category === 'Banki költség') catLabel = 'Banki költség';
          else if (f.category === 'cash_withdrawal' || f.category === 'Banki költség kp felv.') catLabel = 'Banki költség kp felv.';
          else if (f.category === 'other_expense' || f.category === 'other' || f.category === 'Egyéb kiadás' || f.category === 'Egyéb') catLabel = 'Egyéb';
          else if (f.category === 'class_fee') catLabel = 'Osztálypénz';
          else if (f.category) catLabel = f.category;

          cashExpenses.push({
            id: f.id || `exp-${idx + 1}`,
            date: f.date || '2026/2027. tanév',
            description: f.note || f.title || `${catLabel} kiadás`,
            amount: amt,
            formattedAmount: amt.toLocaleString('hu-HU') + ' Ft',
            category: catLabel
          });
    ...
    const closingCash = openingCash + totalIncome - totalExpenses;
    const summary = this.getBankSummaryY3();
    const grandTotal = summary.closingOtpBalance + closingCash;

    return {
      openingCashBalance: openingCash,
      formattedOpeningCashBalance: openingCash.toLocaleString('hu-HU') + ' Ft',
      incomes: cashIncomes,
      expenses: cashExpenses,
      totalIncome: totalIncome,
      formattedTotalIncome: totalIncome.toLocaleString('hu-HU') + ' Ft',
      totalExpenses: totalExpenses,
      formattedTotalExpenses: totalExpenses.toLocaleString('hu-HU') + ' Ft',
      closingCashBalance: closingCash,
      formattedClosingCashBalance: closingCash.toLocaleString('hu-HU') + ' Ft',
      grandTotalClosingBalance: grandTotal,
      formattedGrandTotalClosingBalance: grandTotal.toLocaleString('hu-HU') + ' Ft'
    };
  },
  ...
  getBankSummaryY3() {
    ...
    const expenseCategories = {
      ...
      other_expense: { name: 'Egyéb', total: 0 }
    };

    let totalExpenseAmt = 0;
    let bankExpenseAmt = 0;
    finances.forEach(f => {
      if (f.type === 'expense') {
        const amt = Number(f.amount) || 0;
        totalExpenseAmt += amt;
        ...
        const cat = (f.category === 'Egyéb' || f.category === 'Egyéb kiadás' || f.category === 'other') ? 'other_expense' : (f.category || 'other_expense');
    ...
    return {
      ...
      bankIncome: bankIncomeAmt,
      totalExpenses: totalExpenseAmt,
      bankExpenses: bankExpenseAmt,
      closingOtpBalance: 1247421 + (bankIncomeAmt > 0 ? bankIncomeAmt : effectiveTotalIncome) - bankExpenseAmt
    };
  },
  ...
  _getItemTimestamp(item) {
    if (!item) return 0;
    if (item.createdAt) {
      const t = new Date(item.createdAt).getTime();
      if (!isNaN(t) && t > 0) return t;
    }
    if (typeof item.id === 'string' && item.id.startsWith('fin-')) {
      const num = parseInt(item.id.replace('fin-', ''), 10);
  ...
  getFinances() {
    ...
    try {
      ...
      return list.sort((a, b) => this._getItemTimestamp(b) - this._getItemTimestamp(a));
    } catch {
      return [];
    }
  },
  ...
  async addFinance(item) {
    ...
    list.unshift(newItem);
    list.sort((a, b) => this._getItemTimestamp(b) - this._getItemTimestamp(a));
